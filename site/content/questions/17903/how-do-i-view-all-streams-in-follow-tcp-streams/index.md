...
<table style="width:100%;"><colgroup><col style="width: 50%" /><col style="width: 50%" /></colgroup><tbody><tr class="odd"><td style="width: 30px; vertical-align: top"><div class="vote-buttons"><div id="post-17906-score" class="post-score" title="current number of votes">2</div></div></td><td><div class="item-right"><div class="answer-body"><p>A little bit of tshark scripting is your friend:</p><pre><code>END=$(tshark -r http.pcap -T fields -e tcp.stream | sort -n | tail -1); for ((i=0;i&lt;=END;i++)); do echo $i; tshark -r http.pcap -qz follow,tcp,ascii,$i &gt; follow-stream-$i.txt;done</code></pre><p>... or expanded to a couple of lines for readability:</p><pre><code>END=$(tshark -r ../pcap/http.cap -T fields -e tcp.stream | sort -n | tail -1)
for ((i=0;i&lt;=END;i++))
do
    echo $i

...
<table style="width:100%;"><colgroup><col style="width: 50%" /><col style="width: 50%" /></colgroup><tbody><tr class="odd"><td style="width: 30px; vertical-align: top"><div class="vote-buttons"><div id="post-56301-score" class="post-score" title="current number of votes">1</div></div></td><td><div class="item-right"><div class="answer-body"><p>I totally agree with that explanation. The DNS client's timeout for arrival of a DNS response does exist, and in your case it is shorter than the server's response time. After expiration of that timeout the client unbinds from the socket, causing <code>icmp destination unreachable</code> to be sent in response to anything arriving to that socket, including a valid DNS response.</p><p>If you are interested in details, you can create a batch file like</p><pre><code>@echo off

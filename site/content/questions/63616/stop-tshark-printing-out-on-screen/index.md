...
<table style="width:100%;"><colgroup><col style="width: 50%" /><col style="width: 50%" /></colgroup><tbody><tr class="odd"><td style="width: 30px; vertical-align: top"><div class="vote-buttons"><div id="post-63617-score" class="post-score" title="current number of votes">2</div></div></td><td><div class="item-right"><div class="answer-body"><p>I'm not familiar with using <code>-T ek</code>, but if you know the AVP value you're looking for, you can get it with <code>-T fields</code>. For example, suppose you want the <em>Session-Id</em>:</p><pre><code>tshark -r diameter.pcap -Y &quot;diameter.Session-Id&quot; -T fields -e &quot;diameter.Session-Id&quot;</code></pre><p>If you want the value of all AVP's, that's a little harder. I suppose you could run through the file multiple times for each AVP? I don't know what you're trying to do, but here's a script that does that, which may or may not help you:</p><pre><code>#!/bin/sh
# Check usage
if (( ${#} &lt; 1 ))
then
        echo &quot;Usage: $0 &lt;file&gt;&quot;

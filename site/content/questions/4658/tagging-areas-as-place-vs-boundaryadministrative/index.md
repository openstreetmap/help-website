...
<p>And if that's true, then how far away are we from saying that the place=* schema should be applied to nodes only and boundary schema should be applied to polygons? The <a href="https://wiki.openstreetmap.org/wiki/Key:place#As_areas">As areas</a> portion of the Place key wiki page seems to come close to recommending just that, but it can be interpreted as saying we should be applying place=* tags to settlement boundaries that do not coincide with admin areas.</p>
</div>
